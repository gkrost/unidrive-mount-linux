...
| Title | Scope |
|---|---|
| Write durability: `create`/write success doesn't guarantee the bytes reach the cloud | Surfaced during live mv/cp test-drive. `echo x > /mount/foo` returns success the instant the local cache file exists; the cloud upload fires at FUSE RELEASE via `hydration.open_write`, and the Rust `release` handler only `tracing::warn!`s on `open_write` failure (the kernel has already returned 0 from `close(2)`, and FUSE doesn't read release error codes). So a failed upload is invisible to the user. Compounding: the mount's `fsync` impl flushes the **local cache FD only** (`sync_data`/`sync_all`), it does NOT trigger or await a cloud upload — so even an app that `fsync`s for durability gets no cloud guarantee. Partial mitigation: the cache file persists locally and `cache_scanner::scan_and_replay` re-fires `open_write` for cache files whose mtime exceeds the `last_synced` watermark on the next daemon-restart+mount — but `create` stamps `last_synced=now`, so a just-created file's mtime ≈ watermark and may not be re-detected; recovery is uncertain. Not silent data **loss** (cache survives) but silent cloud-durability **failure** with no operator signal. Fix-shape decision required (pick one or combine): (a) make the mount's `fsync` trigger a synchronous `open_write` + await it, giving fsync-ing apps real cloud durability; (b) on `release`-upload failure, mark the path's state.db row "dirty/unsynced" and surface via an event or a `unidrive doctor` check so the operator learns of the gap; (c) tighten `scan_and_replay`'s watermark comparison so create-then-crash is reliably replayed. `mount/src/fuse_fs.rs` (release/fsync), `mount/src/cache_scanner.rs`, cross-repo `HydrationImpl.openForWrite`. |
